 ...
 * to a Pulse category, crossing from Mayatara into the Opportunities room.
...
export function RelatedOpportunities({ category }: { category: string }) {
  ...
  useEffect(() => {
    fetch(`/api/mayatara/pulse/related-opportunities?category=${encodeURIComponent(category)}`)
      .then((r) => r.json())
