...
export default async function DigestPage({ params }: { params: Promise<{ period: string }> }) {
  ...
  const pageUrl = `${SITE_URL}/mayatara/pulse/digest/${period}`;
    ...
    <div className="mayatara-scope min-h-screen flex flex-col" style={{ background: "var(--bg)" }}>
      <header className="border-b-2 z-10 relative" style={{ borderColor: "var(--border)", background: "var(--card)" }}>
        <div className="max-w-4xl mx-auto px-6 py-3 flex items-center justify-between flex-wrap gap-y-2">
          <Link href="/mayatara/pulse" className="font-typewriter text-xl tracking-wider" style={{ color: "var(--saffron)" }}>MAYATARA · THE PULSE</Link>
      ...
      <div className="flex-1 max-w-3xl mx-auto w-full px-4 py-14 relative z-10">
        <div className="mb-6">
          <Link href="/mayatara/pulse" className="text-xs font-typewriter" style={{ color: "var(--ink-muted)" }}>← Back to Pulse</Link>
        </div>
