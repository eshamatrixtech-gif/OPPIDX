...
import { ApsaraDancer, LoveCouple, LotusBlossom } from "@/components/ui/SculptureAnim";
  ...
  return (
    <div className="stamped-scope min-h-screen flex flex-col" style={{ background: "var(--bg)" }}>
      <div className="w-full text-center py-1 text-xs tracking-widest font-typewriter"
        style={{ background: "var(--maroon)", color: "var(--btn-text)" }}>
      ...
      <header className="relative z-10 border-b-2" style={{ borderColor: "var(--border)", background: "var(--card)" }}>
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between flex-wrap gap-y-3">
          <div className="flex items-center gap-3">
            <Image src="/logo.png" alt="OppIDX" width={88} height={88} style={{ objectFit: "contain", width: "auto" }} />
            <div>
              <h1 className="font-typewriter text-2xl tracking-wider" style={{ color: "var(--saffron)", lineHeight: 1 }}>OPPIDX MATCH</h1>
          ...
          <nav className="flex items-center gap-3 flex-wrap justify-end">
            {userName ? (
              <>
                ...
                <Link href="/account/dashboard" className="btn-primary text-sm">◆ My Dashboard</Link>
              </>
            ) : (
              <>
                <Link href="/account/register" className="btn-primary text-sm">Create Account</Link>
                <Link href="/account" className="btn-secondary text-sm">Log In</Link>
              </>
            )}
            <Link href="/match/compatibility" className="btn-secondary text-sm">Compatibility Check</Link>
          </nav>
        </div>
      </header>
      ...
      <main className="flex-1 relative z-10">
        ...
        <section className="max-w-4xl mx-auto px-6 pt-16 pb-10 text-center relative overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center mb-10">
            <Link href="/account/register" className="btn-primary text-base px-8 py-3">◆ &nbsp; Create Account — It&apos;s Free</Link>
            <Link href="/match/compatibility" className="btn-secondary text-base px-8 py-3">✦ &nbsp; Check Compatibility</Link>
          </div>
        ...
        <section className="max-w-4xl mx-auto px-6 py-16">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-5 gap-3">
            {TYPES.map((t, i) => (
              <Link key={t.label} href={`/account/register`}
        ...
        <section className="max-w-3xl mx-auto px-6 py-16 text-center">
          ...
          <p className="text-base leading-relaxed mb-4" style={{ color: "var(--ink)" }}>
            OppIDX Match gives you one match. We tell you why. You get their contact.
        ...
        <section className="max-w-2xl mx-auto px-6 pb-20 text-center">
          <div className="card p-10" style={{ borderColor: "var(--saffron)", boxShadow: "6px 6px 0 var(--saffron)" }}>
            ...
            <Link href="/account/register" className="btn-primary text-base px-8 py-3">◆ &nbsp; Let&apos;s Begin</Link>

            {/* Terms + Philosophy — small, visible, before they sign up */}
            <div className="flex gap-3 justify-center mt-6 flex-wrap">
              <Link href="/match/philosophy"
      ...
      <footer className="border-t-2 py-6 text-center text-xs tracking-widest"
        ...
        <Image src="/logo.png" alt="OppIDX" width={32} height={32} style={{ objectFit: "contain", display: "inline-block", marginBottom: "8px" }} /><br />
        OPPIDX MATCH · FOR THE REAL ONES · FREE FOREVER · MADE IN INDIA<br />
        <span className="inline-block mt-1 opacity-60">DESIGNED BY SILICON VALLEY ALUMNI</span><br />
        <span className="mt-1 inline-block">
          <Link href="/account/register" style={{ color: "var(--saffron)", textDecoration: "none" }}>Create Account</Link>
          &nbsp;·&nbsp;
          <Link href="/account" style={{ color: "var(--ink-muted)", textDecoration: "none" }}>Log In</Link>
          &nbsp;·&nbsp;
          <Link href="/match/compatibility" style={{ color: "var(--ink-muted)", textDecoration: "none" }}>Compatibility Check</Link>
          &nbsp;·&nbsp;
          <Link href="/match/terms" style={{ color: "var(--ink-muted)", textDecoration: "none" }}>Terms & Privacy</Link>
          &nbsp;·&nbsp;
          <Link href="/match/philosophy" style={{ color: "var(--ink-muted)", textDecoration: "none" }}>Our Philosophy</Link>
        </span>
      </footer>
    </div>
  );
