...
function CommentRow({ comment }: { comment: Comment }) {
  ...
  async function report() {
    ...
    try {
      await fetch("/api/mayatara/pulse/comments/report", {
        method: "POST",
...
export function PulseComments({ headlineId }: { headlineId: string }) {
  ...
  useEffect(() => {
    fetch(`/api/mayatara/pulse/comments?headlineId=${encodeURIComponent(headlineId)}`)
      .then((r) => r.json())
      .then((data) => {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setPostError("");
    try {
      const res = await fetch("/api/mayatara/pulse/comments", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
